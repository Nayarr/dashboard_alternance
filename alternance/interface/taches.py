...
def _executer(tache_id, commande, total, extraire_progression=None):
    ...
    """
    try:
        # Tout passe par cli.py : les noms utilises ici sont exactement
        # ceux que l'utilisateur tape dans un terminal.
        processus = subprocess.Popen(
            [sys.executable, "-u", "cli.py"] + commande,
...
def lancer(type_, parametres):
    ...
    try:
        if type_ == "collecte":
            # Sans --source, la collecte interroge ses cinq sources. La liste
            ...
            commande = ["lettres", "--limite", str(limite)]
            total = limite
