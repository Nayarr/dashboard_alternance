...
connecter_compte.py et relue ici. Aucun mot de passe n'est manipule.
...
Sans --confirmer, le script remplit tout, capture l'ecran et n'envoie rien.
    ...
    python postuler_wttj.py --limite 3
    python postuler_wttj.py --limite 10 --confirmer
"""
...
def postuler(page, offre, lettre, confirmer):
    ...
    if "authenticate" in page.url or "signin" in page.url:
        return "erreur", "session expiree, relancer connecter_compte.py wttj"
...
def main():
    ...
    if not SESSION.exists():
        raise SystemExit("session absente : " + str(SESSION)
                         + "\nLancer : python connecter_compte.py wttj")
    if not CV.exists():
    ...
    if not offres:
        raise SystemExit("aucune offre WTTJ avec lettre prete. "
                         "Lancer d'abord : python generer_lettres.py")
